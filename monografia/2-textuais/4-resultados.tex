...
Os testes foram realizados em dois ambientes: testes internos com o veículo estacionário para validação de comunicação e telemetria, e testes externos em área de aproximadamente 700~m² para operação do veículo em movimento.
...
A \autoref{tab:latencia-udp-medida} apresenta as latências de \texttt{sendto()} medidas no sistema após as correções de resolução \gls{mDNS} e desativação do gerenciamento de energia WiFi descritas na \autoref{subsec:evolucao-decisoes-comunicacao}. Os valores correspondem ao tempo entre a chamada \texttt{sendto()} e seu retorno, medidos via relógio monotônico do sistema operacional com precisão de nanossegundos.
...
\Fonte{elaborado pelo autor.}%
\Nota{* p $<$ 0.05, ** p $<$ 0.001. N = 150.000 pontos de dados.}%
}
\end{table}
...
A \autoref{tab:performance-encoders} apresenta as métricas de performance dos encoders LPD3806-600BM conectados ao ESP32. Os resultados demonstram a precisão e responsividade do sistema de entrada do simulador.
...
\IBGEtab{}{%
  \begin{tabular}{p{4cm}p{2.5cm}p{2.5cm}p{2.5cm}p{2.5cm}}
      ...
      \midrule
      ESP32 DevKit V1 & R\$ 45 & 1 & R\$ 45 & Simulador \\
      \midrule
