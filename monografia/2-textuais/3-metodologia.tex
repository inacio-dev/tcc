...
O projeto seguirá uma arquitetura de três camadas: o veículo teleoperado com o elemento principal sendo o Raspberry Pi 4 terá o objetivo de receber mensagens de controle e enviar mensagens de vídeo e status de sensores; a comunicação entre o veículo e o cliente será feita por protocolo UDP; e, por último, o PC (cliente) receberá as mensagens e encaminhará os comandos de status para o simulador, bem como encaminhará as mensagens de controle para o veículo. A integração de múltiplos sensores segue a abordagem de \citeonline{bobrovsky2023development}, visando um bom resultado de feedback ao usuário. A \autoref{fig:arquitetura-projeto} apresenta o diagrama da arquitetura proposta.
...
Esta seção apresenta as especificações detalhadas dos componentes utilizados no sistema, organizados por subsistema: veículo teleoperado (\autoref{tab:componentes-veiculo}), simulador de controle (\autoref{tab:componentes-simulador}) e estação cliente (\autoref{tab:componentes-cliente}).

\subsection{Componentes do Veículo (Raspberry Pi 4)}
...
\IBGEtab{}{%
	\begin{tabular}{p{3.5cm}p{6cm}p{6cm}}
		...
		\midrule
		Ponte H BTS7960 & 5.5--27Vdc, corrente contínua 40A, proteção térmica integrada & Controle bidirecional de velocidade do motor de propulsão \\
		\midrule
		...
		\midrule
		ACS758 50A (2x) & Sensor Hall, sensibilidade 40mV/A, largura de banda 120kHz & Medição de corrente do Raspberry Pi e servos via ADS1115 \\
		\midrule
...
\subsection{Componentes do Simulador (ESP32)}
...
\IBGEtab{}{%
	\begin{tabular}{p{3.5cm}p{6cm}p{6cm}}
		...
		Componente & Especificação & Função no Sistema \\
		\midrule \midrule
		ESP32 DevKit V1 & Dual-core Xtensa LX6 @ 240MHz, 520KB SRAM, 4MB Flash, WiFi/Bluetooth & Controle de encoders, force feedback via BTS7960 e comunicação serial USB com cliente PC \\
		\midrule
		...
		\midrule
		Ponte H BTS7960 & 5.5--27Vdc, corrente contínua 40A, proteção térmica integrada & Controle bidirecional do motor de force feedback \\
		\midrule
...
Para o simulador, o ESP32 DevKit V1 foi mantido devido ao seu processamento superior em relação ao Arduino Mega, com dual-core de 240MHz contra single-core de 16MHz. Essa capacidade permite leitura simultânea de três encoders rotacionais a 100Hz, processamento de comandos de force feedback e comunicação serial USB com o cliente PC sem perda de dados. A comunicação serial USB entre ESP32 e cliente elimina a necessidade de protocolos wireless adicionais no simulador, garantindo conexão estável e determinística.
