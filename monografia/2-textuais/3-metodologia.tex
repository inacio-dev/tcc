...
\noindent onde $T_{base} = 50s$ é o tempo base para atingir 100\% do PWM (equivalente a 2\%/s), e $M_{zona}$ é o multiplicador da zona de eficiência: $M_{IDEAL} = 1.0$, $M_{SUB} = 0.1$ (10× mais lento), e $M_{RUIM} = 0.04$ (25× mais lento). Isso resulta em taxas de 2\%/s na zona IDEAL, 0,2\%/s na zona SUBÓTIMA e 0,08\%/s na zona RUIM, conforme ilustrado na \autoref{fig:taxa-aceleracao-zonas}.
...
A \autoref{fig:zonas-eficiencia-marchas} ilustra as zonas de eficiência por marcha. Cada marcha possui uma zona IDEAL (verde) onde a aceleração ocorre em taxa normal, uma zona SUBÓTIMA (laranja) com aceleração 10 vezes mais lenta, e uma zona RUIM (vermelho) com aceleração 25 vezes mais lenta. Os limitadores de PWM restringem a potência máxima disponível em cada marcha com intervalos crescentes: 12\% na 1ª, 25\% na 2ª, 45\% na 3ª, 70\% na 4ª e 100\% na 5ª marcha. Todas as marchas possuem uma zona morta de 0 a 6\% do PWM (cinza), faixa na qual o motor não possui torque suficiente para movimentar o veículo, sendo atravessada automaticamente por uma rampa rápida com multiplicador de 20× quando o operador solicita aceleração. As zonas IDEAL de cada marcha são contíguas entre si, garantindo que a progressão de marchas cubra toda a faixa de PWM sem lacunas. Esse sistema força o operador a progredir pelas marchas para atingir velocidade máxima, reproduzindo o comportamento de uma transmissão real.
