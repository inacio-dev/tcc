...
O projeto seguirá uma arquitetura de três camadas. A primeira corresponde ao veículo teleoperado, tendo como elemento principal o Raspberry Pi 4, responsável por receber mensagens de controle e enviar mensagens de vídeo e status de sensores. A segunda camada é a comunicação entre o veículo e o cliente, realizada por protocolo UDP. A terceira camada é o PC (cliente), que receberá as mensagens e encaminhará os comandos de status para o simulador, bem como encaminhará as mensagens de controle para o veículo. A integração de múltiplos sensores segue a abordagem de \citeonline{bobrovsky2023development}, visando um bom resultado de feedback ao usuário. A \autoref{fig:arquitetura-projeto} apresenta o diagrama da arquitetura proposta.
...
O Raspberry Pi 4 Model B com 8GB de RAM constitui o núcleo de processamento embarcado no veículo, responsável por todas as operações de controle, sensoriamento e comunicação. A escolha deste hardware foi motivada pela capacidade de processamento suficiente para codificação de vídeo MJPEG em tempo real, comunicação UDP nativa e controle de múltiplos periféricos via I2C, conforme validado nos testes de arquitetura descritos na \autoref{subsec:evolucao-decisoes-comunicacao}.
...
O motor RC 775 opera com transmissão manual de 5 marchas, simulando comportamento de veículo real. O controle de velocidade é realizado via PWM através do driver BTS7960. O algoritmo de controle ajusta o fator de torque conforme a marcha selecionada:
...
O sistema implementa zonas de eficiência inspiradas em veículos de Fórmula 1, onde cada marcha possui uma faixa ideal de operação em termos de PWM do motor. Operar fora dessa faixa resulta em penalização na taxa de aceleração, incentivando o operador a utilizar a marcha correta para cada situação. A \autoref{fig:zonas-eficiencia-marchas} ilustra as zonas de eficiência e os limitadores de PWM por marcha.

\begin{figure}[!ht]
	\captionsetup{width=16cm}
	\Caption{\label{fig:zonas-eficiencia-marchas} Sistema de transmissão F1: (a) zonas de eficiência por marcha e (b) limitadores de PWM com fator de torque}
	\UFCfig{}{
		\includegraphics[width=15cm]{figuras/sistema_transmissao_f1}
	}{
		\Fonte{elaborado pelo autor.}
	}
\end{figure}

Conforme ilustrado na figura, cada marcha possui uma zona IDEAL (verde) onde a aceleração ocorre em taxa normal, uma zona SUBOPTIMAL (laranja) com aceleração 10 vezes mais lenta, e uma zona POOR (vermelho) com aceleração 25 vezes mais lenta. Os limitadores de PWM restringem a potência máxima disponível em cada marcha: 40\% na 1ª, 60\% na 2ª, 80\% na 3ª e 100\% na 4ª e 5ª marchas. Esse sistema força o operador a progredir pelas marchas para atingir velocidade máxima, reproduzindo o comportamento de uma transmissão real.
...
O cliente PC, implementado em Python com interface Tkinter, atua como hub central do sistema, recebendo dados do veículo via UDP, processando telemetria, calculando force feedback e transmitindo comandos para simulador e veículo. O Python foi escolhido devido à facilidade de prototipagem e disponibilidade de bibliotecas para decodificação de vídeo (PyAV), interface gráfica (Tkinter) e comunicação (socket, pyserial).
...
O painel de instrumentos exibe RPM, marcha atual, posição do acelerador e velocidade estimada, atualizados em tempo real conforme dados recebidos do veículo e simulador. A visualização de vídeo utiliza OpenCV para decodificação MJPEG e Pillow para renderização no canvas Tkinter.
