...
A análise comparativa dos testes (\autoref{tab:comparativo-comunicacao}) definiu a arquitetura final: protocolo UDP, Raspberry Pi 4 embarcado no veículo e simulador Logitech G923.
...
A decisão de embarcar o Raspberry Pi 4 diretamente no veículo eliminou essas limitações, permitindo comunicação UDP nativa sobre WiFi. Com o Raspberry Pi embarcado, a captura de vídeo, codificação MJPEG e transmissão UDP ocorrem internamente no mesmo dispositivo, o que simplifica a arquitetura. O Raspberry Pi atua simultaneamente como controlador do veículo e servidor UDP, processando vídeo da câmera OV5647, dados do sensor BMI160 e comandos de controle em uma única plataforma. Para simplificar a descoberta de dispositivos na rede local, foi adotado o protocolo mDNS, permitindo que o veículo seja acessado pelo nome \texttt{f1car.local} e o cliente por \texttt{f1client.local}, eliminando a necessidade de configuração manual de endereços IP. Para evitar bloqueios durante a transmissão, duas otimizações foram aplicadas à camada de rede. A primeira é a resolução antecipada do \textit{hostname} mDNS: ao estabelecer conexão, o \textit{hostname} \texttt{f1client.local} é resolvido uma única vez para endereço IP numérico via \texttt{socket.gethostbyname()}, e todas as chamadas \texttt{sendto()} subsequentes utilizam o IP em cache. Sem essa otimização, cada chamada \texttt{sendto()} com \textit{hostname} dispara resolução mDNS pelo daemon Avahi, que possui \textit{timeout} padrão de 5 segundos --- com 160 pacotes por segundo (100Hz de sensores e 60Hz de vídeo), qualquer expiração do \textit{cache} mDNS bloqueava ambos os \textit{sockets} simultaneamente por 5 segundos. A re-resolução periódica (a cada 10 segundos) garante atualização do IP em caso de mudança pelo DHCP. A segunda otimização é a desativação do gerenciamento de energia WiFi do Raspberry Pi 4: o \textit{driver} \texttt{brcmfmac} ativa por padrão o modo de economia de energia, desligando periodicamente a antena WiFi e bloqueando chamadas \texttt{sendto()} até a reativação. A configuração permanente via NetworkManager (\texttt{wifi.powersave = 2}) elimina esses bloqueios.
...
\IBGEtab{}{%
	\begin{tabular}{ccp{5cm}ccc}
		...
		\midrule \midrule
		9999 & RPi $\rightarrow$ PC & Vídeo MJPEG + energia + temperatura & 60Hz & 10--50 KB & 64 KB \\
		\midrule
...
\IBGEtab{}{%
	\begin{tabular}{p{3.5cm}p{6cm}p{6cm}}
		...
		\midrule
		Câmera OV5647 & 5MP, 2592×1944 pixels, vídeo até 1080p/30fps, interface CSI & Captura de vídeo em 640×480/60fps para transmissão via UDP \\
		\midrule
		Sensor BMI160 \cite{bosch2015bmi160} & \gls{IMU} 6 eixos, ±2g/±250°/s, 16 bits, I2C, 100Hz sampling & Detecção de \glspl{forcag}, aceleração e velocidade angular para force feedback \\
		\midrule
		...
		\midrule
		PCA9685 PWM Driver \cite{nxp2015pca9685} & 16 canais, 12 bits, I2C, endereço \texttt{0x41} (A0 soldado) & Controle de servos (freio dianteiro, traseiro e direção) \\
		\midrule
		Arduino Pro Micro & ATmega32U4, 16MHz, ADC 10 bits, USB nativo & Leitura de sensores ACS758 e tensão da bateria via USB serial \\
		\midrule
		INA219 \cite{ti2015ina219} & Sensor corrente/tensão, I2C, endereço \texttt{0x40} & Monitoramento de energia do Raspberry Pi \\
		\midrule
...
O veículo possui múltiplos sensores para captura de dados em tempo real, transmitidos ao cliente via UDP para processamento e geração de feedback háptico.

\subsubsection{Câmera OV5647}

A câmera OV5647 conectada via interface CSI captura vídeo em resolução 640x480 a 60 \gls{FPS}. O stream de vídeo é codificado em MJPEG (Motion JPEG), onde cada frame é comprimido independentemente como imagem JPEG. Essa escolha foi motivada pela robustez contra perda de pacotes UDP: enquanto codecs como H.264 utilizam P-frames que dependem de frames anteriores (causando distorção em cascata quando há perda de pacotes), o MJPEG transmite frames independentes, limitando o impacto de perdas a um único frame. O trade-off é maior uso de banda (aproximadamente 4x mais que H.264), compensado pela qualidade de imagem consistente e simplicidade de decodificação via OpenCV.
...
O BMI160 é suscetível a \textit{brown-out} elétrico durante picos de corrente do motor DC 775, que causam quedas momentâneas de tensão no barramento 3,3V do Raspberry Pi. Quando isso ocorre, o sensor realiza um \textit{power-on reset} interno e entra em modo \textit{suspend}, onde os registradores de dados retornam zero indefinidamente apesar de o barramento I2C continuar funcionando. Para mitigar esse problema, o software implementa um \textit{watchdog} que monitora leituras consecutivas zeradas. Após 3 leituras zeradas consecutivas (30ms a 100Hz), o sistema executa uma rotina de recuperação que reativa a unidade de gerenciamento de energia (PMU) do sensor, reconfigura os parâmetros de faixa e taxa de amostragem, e verifica o sucesso da recuperação. Os tempos de recuperação seguem as especificações do \textit{datasheet} BST-BMI160-DS000-09: 3,8ms para ativação do acelerômetro e 80ms para o giroscópio. Caso a reativação da PMU falhe, o sistema executa um \textit{soft reset} completo do sensor como segunda tentativa de recuperação.

\subsubsection{Monitoramento de Temperatura}

O sensor digital DS18B20 \cite{maxim2019ds18b20} conectado via protocolo 1-Wire (GPIO4) monitora a temperatura do motor e da eletrônica embarcada a 1Hz. O sensor possui faixa de $-55$°C a $+125$°C com precisão de $\pm0{,}5$°C e validação de dados por CRC. O software implementa alertas térmicos em três níveis: aviso a 65°C, crítico a 80°C e desligamento automático a 85°C. Os dados de temperatura são incluídos nos pacotes UDP da porta 9999 e exibidos na interface do cliente.

\subsubsection{Monitoramento de Energia}

O sistema de monitoramento de energia utiliza uma arquitetura híbrida com dois subsistemas complementares: um Arduino Pro Micro (ATmega32U4) dedicado à leitura dos sensores de corrente ACS758 e tensão da bateria, e um sensor INA219 (endereço I2C \texttt{0x40}) para monitoramento de corrente e tensão do Raspberry Pi.
...
O sistema de propulsão utiliza motor DC 775 de 24V controlado por ponte H BTS7960, enquanto direção e freios são acionados por servos MG996R através do driver PWM PCA9685 (endereço I2C \texttt{0x41}, com pino A0 soldado para evitar conflito com o INA219 no endereço padrão \texttt{0x40}).
...
A arquitetura multi-thread implementada utiliza sete threads. Quatro são de aquisição: câmera a 60Hz (captura de frames da OV5647), sensores a 100Hz (leitura do BMI160), energia a 10Hz (Arduino Pro Micro via USB serial e INA219 via I2C) e temperatura a 1Hz (DS18B20 e métricas do sistema operacional como uso de CPU, memória RAM, disco e rede). Duas são de transmissão: vídeo a 60Hz, que consolida frame MJPEG com dados de energia, temperatura e métricas do sistema e envia pela porta UDP 9999; e sensores a 100Hz, que envia dados do BMI160 pela porta UDP 9997. A sétima thread é a de recepção de comandos, que opera como daemon no gerenciador de rede pela porta UDP 9998. Essa separação evita que a captura de vídeo atrase a leitura de sensores e vice-versa, reduzindo a latência de sensores de aproximadamente 16ms (espera por frame a 60Hz) para aproximadamente 10ms (envio direto pela porta dedicada). Para diagnóstico de latência, cada pacote UDP inclui campos de instrumentação de tempo (\texttt{timing\_bmi160\_read}, \texttt{timing\_lock}, \texttt{timing\_total\_pre\_send}), que medem a duração de cada etapa do pipeline.
